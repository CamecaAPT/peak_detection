...
//
// Load it via the console's "Load…" button, then Run (or Ctrl+Enter).

// --- File-chooser ("…" browse button) for the RunConfig path in the property grid. ---
// These #r references must stay at the very top of the script (before any other code) and
// rely on a standard WinForms PropertyGrid honoring UITypeEditor (.NET Framework host). If the
// script fails to load on these references, or no browse button appears (the console uses a
// custom settings UI), comment out the two #r lines, the two `System.*.Design` usings, and the
// [Editor(...)] attribute on RunConfigPath — the field then renders as a plain editable textbox
// (you can still type/paste a path).
#r "System.Drawing"
#r "System.Design"
...
var extRoot    = @"C:\workspace\extensions\peak_detection";
